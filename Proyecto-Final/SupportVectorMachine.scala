...
val data = spark.read.option("header","true").option("inferSchema","true").option("delimiter",";").format("csv").load("/home/saulsp96/Downloads/bank/bank-full.csv")
...
val dt = new LinearSVC().setLabelCol("indexedLabel").setFeaturesCol("indexedFeatures")
//Rama de prediccion
...
val treeModel = model.stages(2).asInstanceOf[LinearSVC]
